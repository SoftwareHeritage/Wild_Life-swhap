...
unsigned int global_time_stamp=INIT_TIME_STAMP; /* 9.6 */
#endif
...
int ignore_eff; /* 'Ignore efficiency' flag */
...
int more_u_attr; /* TRUE if U has attributes V doesn't */
int more_v_attr; /* Vice-versa */

int u_func,v_func;  /* TRUE if U or V is a curried function */
int new_stat;
...
def_type typ;
{
  ...
  if (redefine(head)) {
    ...
    if (def->type==undef || def->type==typ) {
      if (def->rule && (int)def->rule<=MAX_BUILT_INS) {
...
ptr_psi_term t;
{
  ...
  if (equ_tok((*t),"alias")) {
    ...
    if (arg1 && arg2) {
      Warningline("'%s' now means the same as '%s'.\n",
                  arg1->type->symbol, arg2->type->symbol);
      str=arg2->type->symbol;
      assert_ok=TRUE;
      deref_ptr(arg1);
      deref_ptr(arg2);
      *(arg2->type)= *(arg1->type);
      arg2->type->symbol=str;
    }
    else
...
{
  /*
    while((int)undo_stack>(int)goal_stack)
  */
  
  while ((int)undo_stack>(int)limit) { 
#ifdef X11
    if (undo_stack->type & undo_action) {
      /* Window operation on backtracking */
      if (undo_stack->type==destroy_window) {
        x_destroy_window((int)undo_stack->a,(int)undo_stack->b);
      }
      else if (undo_stack->type==show_window) {
        x_show_window((int)undo_stack->a,(int)undo_stack->b);
      }
      else if (undo_stack->type==hide_window) {
        x_hide_window((int)undo_stack->a,(int)undo_stack->b);
      }
    }
    else
#endif
  ...
  while ((int)u) {
    if (u->type & undo_action) {
      if (u->type==destroy_window) {
        x_destroy_window((int)u->a,(int)u->b);
      }
      else if (u->type==show_window) {
        x_show_window((int)u->a,(int)u->b);
      }
      else if (u->type==hide_window) {
        x_hide_window((int)u->a,(int)u->b);
      }
    }
    u=u->next;
  }
...
ptr_choice_point cutpt;
{
  ...
  while ((int)u > (int)cut_limit) {
    clean_iter++;
    if (!(u->type & undo_action) && VALID_RANGE(u->a) &&
        (int)u->a>(int)cut_sp && (int)u->a<=(int)stack_pointer) {
      *prev = u->next;
      clean_succ++;
    }
    prev = &(u->next);
...
void show_count()
{
  ...
  if (verbose) {
    ...
    printf("%1.3fs cpu, %d goal%s",t,goal_count,(goal_count!=1?"s":""));
...
/******** FETCH_DEF_LAZY(psi_term,type1,type2,attr_list1,attr_list2)
  ...
  pred     = thing(X)
*/
void fetch_def_lazy(u, old1, old2, old1attr, old2attr)
ptr_psi_term u;
...
int unify_body(eval_flag)
int eval_flag;
{
  int success=TRUE,compare;
  ...
  if (u!=v) {
	...
	success=(r==floor(r));
      ...
      if (success) {
        ...
        if (u->value && !v->value && overlap_type(new_type,alist)) {
           lu=(ptr_list)u->value;
           if (lu->cdr) deref2_rec_eval(lu->cdr);
           if (lu->car) deref2_rec_eval(lu->car);
        }
        if (v->value && !u->value && overlap_type(new_type,alist)) {
           lv=(ptr_list)v->value;
           if (lv->cdr) deref2_rec_eval(lv->cdr);
           if (lv->car) deref2_rec_eval(lv->car);
        }
	...
	if (u->value!=v->value)
	  if (!u->value) {
	    ...
	    u->value=v->value;		
            ...
            if (overlap_type(new_type,alist)) {
              lu=(ptr_list)u->value;
              lv=(ptr_list)v->value;
              if (lu->car && lv->car) {
                if (lu->cdr!=lv->cdr) {
                  if (lu->cdr==NULL) {
                    push_ptr_value(psi_term_ptr,&(lu->cdr));
                    lu->cdr=stack_empty_list();
                    /* lu->cdr=stack_copy_psi_term(*empty_list); 5.8 */
                  }
                  if (lv->cdr==NULL) {
                    push_ptr_value(psi_term_ptr,&(lv->cdr));
                    lv->cdr=stack_empty_list();
                    /* lv->cdr=stack_copy_psi_term(*empty_list); 5.8 */
                  }
                  push_goal(unify,lu->cdr,lv->cdr,NULL);
                }
                if (lu->car!=lv->car)
                  push_goal(unify,lu->car,lv->car,NULL);
              }
              else
                success=(lu->car==lv->car);
            }
            else if (overlap_type(new_type,real))
              success=(*((REAL *)u->value)==(*((REAL *)v->value)));
            else if (overlap_type(new_type,quoted_string))
              success=(strcmp(u->value,v->value)==0);
	...
	if ((u_func && more_v_attr) || (v_func && more_u_attr)) {
	  if (!(u->flags&QUOTED_TRUE | v->flags&QUOTED_TRUE)) { /* 17.9 */
	...
	if (FALSE) {
	  /**** RELEASE RESIDUATIONS **** INCORRECT ****/
          /* This version is disabled by default. */
	  /* It only releases residuations when more information has */
	  /* been brought. */
	  switch (compare) {
	  case 1:
	    if (v->resid)
	...
	/**** VERIFY CONSTRAINTS ****/
        if (new_stat<4 && u->type->type==type) {
          ...
          fetch_def_lazy(u, old1, old2, old1attr, old2attr);
        }
      }
    }
  }
...
int disjunct_aim()
{
  ...
  if (u->type==disjunction) {
    ...
    if (v!=NULL) {
      ...
      if (v) {
        deref_ptr(v);
        /* aim->c==TRUE only on first run-through */
        if ((int)aim->c) {
          if (l->cdr) {
            Traceline("pushing choice point %P, still to do %P\n", u, l->cdr);
	    push_choice_point(disj,u,l->cdr,(GENERIC)TRUE);
          }
          Traceline("setting disjunction %P to %P\n", u, v);
          /* The binding must be done before v is evaluated. */
          if (u==v) {
            /* push_ptr_value(psi_term_ptr,&(u->coref)); */
	    push_psi_ptr_value(u,&(u->coref));
            u->coref=stack_psi_term(4);
          }
          else {
            i_check_out(v); /* For type constraints */
            /* push_ptr_value(psi_term_ptr,&(u->coref)); */
	    push_psi_ptr_value(u,&(u->coref));
            u->coref=v;
          }
        ...
        aim->c=(GENERIC)FALSE;
        /* This will evaluate v, and cause a second run-through. */
        /* If v is a disjunction, it will also create a choice point. */
        /* i_check_out(v); */
        deref(v);
        aim->c=(GENERIC)TRUE;
      }
      ...
      perr("*** Bug: Illegal NULL-terminated disjunction.\n");
      success=FALSE;
    }
  }
...
int prove_aim()
{
  ...
  if (thegoal && rule) {
	...
	if (i_check_out(thegoal)) { /*PVR 13.4 instead of i_check_out*/

	  goal_stack=aim->next;
	  goal_count++;

          if ((int)rule==DEFRULES) {
	  ...
	  if (success) {
	    ...
	    if ((int)rule<=MAX_BUILT_INS) {
	      ...
	      success=c_rule[(int)rule]();

              /* RESPRED */ if (curried)
              /* RESPRED */   do_currying();
              /* RESPRED */ else if (resid_vars)
              /* RESPRED */   success=do_residuation(); /* 21.9 */
	    }
	    else {
...
int what_next_aim()
{
  ...
  if (aim->a) {
    ...
    var_occurred=var_occurred || ((int)aim->b)&TRUEMASK; /* 18.8 */
    eventflag=(((int)aim->b)&(TRUEMASK*2))!=0;
    /* If a window was created, the level should be incremented. */
#ifdef X11
    if (eventflag && !x_window_creation) {
#else
    if (FALSE) {
#endif
      /* If the last query was an event, the level should be decremented */
      /* and no cut done. */
      if (level>0) level--;
    }
#ifdef X11
    else if (!(var_occurred || x_window_creation) && no_choices() && level>0) {
#else
    else if (!(var_occurred) && no_choices() && level>0) {
#endif
  ...
  if (eventflag) {
    ...
    push_goal(what_next,TRUE,FALSE+2*TRUE,level+1);
  ...
  /* prompt_choice_stack=topmost_what_next(); 12.7 */
  /* prompt_choice_stack=NULL; */
  var_occurred=FALSE;
  start_chrono();
...
int load_aim()
{
  int success=TRUE,exitloop;
  ptr_psi_term s;
  int sort;
  char *fn;
  int old_noisy,old_file_date;
  ptr_node old_var_tree;
  ptr_choice_point cutpt;
  int old_var_occurred; /* 18.8 */
