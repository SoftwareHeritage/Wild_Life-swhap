     ...
     char *symbol;
{
  ...
  if(key)
	...
	key=HEAP_ALLOC(struct _keyword);
	key->module=module;
...
/** OBSOLETE
  ...
  if(!result)
  Errorline("error in definition of '%s'",module_buffer);
...
/******** REPLACE(old,new,term)
  Replace all occurences of type OLD with NEW in TERM.
  */
...
/******** C_REPLACE()
  Replace all occurences of type ARG1 with ARG2 in ARG3.
  */
