...
{
  long success=TRUE;
  ptr_psi_term funct,arg1,arg2,arg3,tmp;
  long sm1, sm2, sm3;
  ...
  if (arg1 && arg2) {
	...
	if (a3==UNDEF) residuate(arg3);
      ...
      success=FALSE;
      Errorline("Non-boolean argument or result in '%P'.",funct);
    }
  }
  else
    curry();
...
static long c_load()
{
  ...
  if(arg1) {
	...
	file_date+=2;
	/* } */
        /* open_input_file("stdin"); What ????  RM: Jan 27 1993  */
      }
    }
    else {
...
int global_error_check(n, error, eval)
ptr_node n;
int *error, *eval;
{
  if (n) {
...
static long c_bk_assign()
{
  ...
  if (arg1 && arg2) {
	...
	Errorline("cannot use '<-' on persistent value in %P",g);
	return c_abort();
      }
