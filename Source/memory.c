...
#define ALIGNUP(X) {long c=((long)(X) & (ALIGN-1)); (long)(X) += c?(ALIGN-c):0; }
...
char *GetStrOption(char *name,char *def)
{
  ...
  for(i=1;i<arg_c;i++)
    if(arg_v[i][0]=='-' && strlen(arg_v[i])>=l+1)
...
static void check_module(m)        /*  RM: Jan 12 1993  */
     ...
     ptr_module *m;
{
  if(unchecked(m,sizeof(struct _module))) {
...
static void check_keyword(k)      /*  RM: Jan 12 1993  */
     ...
     ptr_keyword *k;
{
  if(unchecked(k,sizeof(struct _keyword))) {
