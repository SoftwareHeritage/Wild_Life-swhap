...
static int attr_missing;
static int check_func_flag;
...
/* Create an empty list on the stack */
ptr_psi_term stack_empty_list()
{
  ptr_psi_term emp;
  
  emp = stack_psi_term(4);
  emp->type = alist;
  emp->value = (GENERIC)STACK_ALLOC(list);
  ((ptr_list)emp->value)->car=NULL;
  ((ptr_list)emp->value)->cdr=NULL;
...
/******** RESIDUATEGOALONVAR(g,var,othervar)
  ...
  is needed for this.
*/
int residuateGoalOnVar(g, var, othervar)
ptr_goal g;
ptr_psi_term var,othervar;
{
  int result;
  int resflag,resflag2;
...
int do_residuation()
{
  ...
  if (resid_aim->next) {
    push_ptr_value(goal_ptr,&(resid_aim->next));
    resid_aim->next=NULL;
  }
  ...
  if (rule) {
	...
	if (resid_vars)
      ...
      if (rule) {
        ...
        clear_copy();
        if (TRUE) /* 8.9 */
        /* if (funct->type->evaluate_args) 8.9 */
...
void match_list(u,v,rb,success)
ptr_psi_term u,v;
ptr_resid_block rb;
int *success;
{
  ...
  if (lu.cdr!=lv.cdr) {
    if (lv.cdr)
      if (lu.cdr) {
        push_goal(match,lu.cdr,lv.cdr,rb);
        deref2_eval(lu.cdr);
        deref2_eval(lv.cdr);
      }
      else
        push_goal(unify,stack_empty_list(),lv.cdr,NULL);
        /* push_goal(unify,stack_copy_psi_term(*empty_list),lv.cdr,NULL); 5.8 */
    else {
      push_goal(match,lu.cdr,stack_empty_list(),rb);
      /* push_goal(match,lu.cdr,stack_copy_psi_term(*empty_list),rb); 5.8 */
      deref2_eval(lu.cdr);
    }
  }
  ...
  if (lu.car!=lv.car) {
    if (lu.car && lv.car) {
      push_goal(match,lu.car,lv.car,rb);
      deref2_eval(lu.car);
      deref2_eval(lv.car);
    }
    else
...
int match_aim()
{
  ...
  if (u!=v) {
    if (success=matches(u->type,v->type,&lesseq)) {
        ...
        if (u->type!=cut || v->type!=cut) { /* Ignore value field for cut! */
          if (v->value) {
            if (u->value) {
              ...
              else if (overlap_type(v->type,alist))
                match_list(u,v,rb,&success);
      ...
      if (success) {
        if (u<match_date && v<match_date) { /* 21.9 */
          /* residuate2(u,v); 21.9 */
          residuate_double(u,v); /* 21.9 */
          residuate_double(v,u); /* 21.9 */
	}
        else if (v<match_date) {
          residuate_double(v,u); /* 21.9 */
        }
        else {
          v->coref=u;
...
/******** CHECK_CONJ(t)
  ...
  Normal case:
  T is a list [H|Rest]
  bind T to H, check_out(H)
  if Rest != [] then
  check_out(Rest), unify H with Rest.

  Other cases are checked such as T=[] etc... which shouldn't normally
  occur.
*/
int check_conj(t)
ptr_psi_term t;
{
  ...
  Errorline("check_conj called with %P.\n",t); /* 19.8 */
  if (t->value) {
    /* Set the conjunction to its CAR, and */
    /* unify it to its CDR if there is one */
    
    l=(ptr_list)t->value;

    /* push_ptr_value(psi_term_ptr,&(t->coref)); */ /* 9.6 */
    push_psi_ptr_value(t,&(t->coref));
    t->coref=l->car;
    
    if (l->car) {
      if (l->cdr) {
	push_goal(unify,t,l->cdr,NULL);
	check_out(l->cdr);
	check_out(t);
      }
      else
    ...
    /* Conjunction has no list value (the user typed '*conjunction*' */
    /* somewhere which is the inner and theoretically hidden */
...
ptr_psi_term t;
{
  ...
  if (t->type->properties) {
    ...
    /* if (!overlap_type(t->type,list_object)) 26.2 */
    if (!overlap_type(t->type,alist))
...
/******** CHECK_LIST(t)
  T is a list whose elements must be checked out.

  Life would be a lot simpler if I didn't have this silly special data
  structure for lists. I could completely dispense with lists, comma ','
  is a binary constructor which behaves exactly the same way, you just need
  to define comma and nil as sub-types of some list type.
  
  What's more there might just be properties to check out for lists, in
  case someone wrote something nasty like:
  :: [H|T](head => H,tail => T).
*/
int check_list(t)
ptr_psi_term t;
{
  ...
  flag=check_type(t); /* This not only deals with nasty definitions,
			 it also makes sure I won't loop on cyclic lists
			 by setting the status to 2 or 4 */
  l=(ptr_list)t->value;
  if (l && l->car) {
    flag=check_out(l->car) && flag;
    if (l->cdr)
      flag=check_out(l->cdr) && flag;
  }
...
ptr_psi_term t;
{
  ...
  if (t->status)
    ...
    if (t->type==disjunction) {
      if (check_func_flag) check_disj(t);
      flag=TRUE;
    }
    else
      if (t->type->type==function /* && !check_func_flag */) {
        if (check_func_flag) {
          check_func(t);
          flag=TRUE;
        }
        ...
        if (sub_type(t->type,alist))
          flag=check_list(t);
        else
          if (t->type->type==type)
            flag=check_type(t);
          else {
            /* if (t->type->type!=function) t->status=4; */
           flag=eval_args(t->attr_list);
          }
  t->status &= ~RMASK;
  }
...
void deref_rec_list(t)
ptr_psi_term t;
{
  ptr_list l;
  ptr_psi_term car,cdr;

  if (overlap_type(t->type,alist) && (l=(ptr_list)t->value)) {
    if (cdr=l->cdr) {
      deref_ptr(cdr);
      deref_rec_body(cdr);
    }
